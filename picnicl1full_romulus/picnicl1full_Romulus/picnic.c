...
int get_param_set(picnic_params_t picnicParams, paramset_t* paramset)
{
    ...
    // paramset->andSizeBytes = nr ANDs por byte * nr bytes plaintext *
    //                          nr rondas skinny * nr chamadas skinny



    paramset->andSizeBytes = numBytes(2 * 16 * 40 * 2);
                  ...
                  picnic_privatekey_t* sk)
{
    ...
    romulus_t_encrypt((uint8_t*)pk->plaintext, (uint8_t*)pk->ciphertext, (uint8_t*)sk->data, nonce, &paramset);
...
size_t picnic_signature_size(picnic_params_t parameters)
{
    ...
    case TRANSFORM_FS:
        ...
        return paramset.numMPCRounds * (paramset.digestSizeBytes + paramset.stateSizeBytes + numBytes(2*16*40*2) +  2 * paramset.seedSizeBytes) + numBytes(2 * paramset.numMPCRounds) + paramset.saltSizeBytes;
    case TRANSFORM_UR:
        return paramset.numMPCRounds * (paramset.digestSizeBytes + paramset.stateSizeBytes + 2 * numBytes(2*16*40*2) +  3 * paramset.seedSizeBytes) + numBytes(2 * paramset.numMPCRounds) + paramset.saltSizeBytes;
    default:
        return PICNIC_MAX_SIGNATURE_SIZE;
    }
}
...
int picnic_validate_keypair(const picnic_privatekey_t* privatekey, const picnic_publickey_t* publickey)
{
    ...
    uint8_t nonce[] = "000102030405060708090A0B";
    romulus_t_encrypt((uint8_t*)publickey->plaintext, (uint8_t*)ciphertext, (uint8_t*)privatekey->data, nonce, &paramset);
