...
static uint8_t skinny_byte_update(uint8_t x, int player){
    (void) player;
...
static void SubCell8(uint8_t state[4][4]) {
    ...
    for(i = 0; i < 4; i++) {
        for(j = 0; j <  4; j++) {
            state[i][j] = skinny_byte_update(state[i][j],0);
        }
    }
}
...
void msg_encryption (const unsigned char* M, unsigned char* C,
...
void romulus_t_encrypt (const uint8_t* plaintext,
                        uint8_t* output,
                        uint8_t *key,
                        uint8_t *nonce,
                        paramset_t* params)
{
    ...
    const unsigned char* K;
    const unsigned char* N;
    uint8_t *c = malloc(sizeof(uint8_t)*params->stateSizeBytes);
    ...
    msg_encryption(M,c,N,CNT,Z);    
    
    // Chave pública 
    memcpy(output,c,params->stateSizeBytes);
}
...
// Extract and apply the subtweakey to the internal state (must be the two top rows XORed together), then update the tweakey state
void mpc_AddKey_verify(unsigned char state[2][4][4], unsigned char keyCells[2][3][4][4])
{
    int i, j, k, player;
    unsigned char pos;
    unsigned char keyCells_tmp[2][3][4][4];
    ...
    for(player=0;player<2;player++){
        ...
        for(i = 0; i <= 1; i++)
        {
            for(j = 0; j < 4; j++)
            {
                state[player][i][j] ^= keyCells[player][0][i][j] ^ keyCells[player][1][i][j] ^ keyCells[player][2][i][j];            
            }
        }
        ...
        for(k = 0; k <(int)(TWEAKEY_SIZE/BLOCK_SIZE); k++){
            for(i = 0; i < 4; i++){
                for(j = 0; j < 4; j++){
                    //application of the TWEAKEY permutation
                    pos=TWEAKEY_P[j+4*i];
                    keyCells_tmp[player][k][i][j]=keyCells[player][k][pos>>2][pos&0x3];
                }
            }
        }
        ...
        for(k = 0; k <(int)(TWEAKEY_SIZE/BLOCK_SIZE); k++){
            for(i = 0; i <= 1; i++){
                for(j = 0; j < 4; j++){
                    ...
                    if (k==1)
                    {
              keyCells_tmp[player][k][i][j]=((keyCells_tmp[player][k][i][j]<<1)&0xFE)^((keyCells_tmp[player][k][i][j]>>7)&0x01)^((keyCells_tmp[player][k][i][j]>>5)&0x01);
                    }
                    else if (k==2)
                    {
              keyCells_tmp[player][k][i][j]=((keyCells_tmp[player][k][i][j]>>1)&0x7F)^((keyCells_tmp[player][k][i][j]<<7)&0x80)^((keyCells_tmp[player][k][i][j]<<1)&0x80);
                    }
                }
            }
        }
        ...
        for(k = 0; k <(int)(TWEAKEY_SIZE/BLOCK_SIZE); k++){
            for(i = 0; i < 4; i++){
                for(j = 0; j < 4; j++){
                    keyCells[player][k][i][j]=keyCells_tmp[player][k][i][j];
                }
            }
        }
    }
...
// encryption function of Skinny
void mpc_enc_verify(uint8_t input[2][16], const uint8_t tweakeys[2][48], uint8_t output[2][16], randomTape_t* rand, view_t views[2]) {
    ...
    for(int player=0;player<2;player++){
        for(i = 0; i < 16; i++) {
            ...
            state[player][i>>2][i&0x3] = input[player][i]&0xFF;

            keyCells[player][0][i>>2][i&0x3] = tweakeys[player][i]&0xFF;
            keyCells[player][1][i>>2][i&0x3] = tweakeys[player][i+16]&0xFF;
            keyCells[player][2][i>>2][i&0x3] = tweakeys[player][i+32]&0xFF;
        }
    }
    #ifdef DEBUG
    ...
    for(i = 0; i < N_RNDS; i++) {
        ...
        mpc_AddConstants_verify(state, i);
            ...
            #endif
        mpc_AddKey_verify(state, keyCells);
            #ifdef DEBUG
    ...
    for(int player=0;player<2;player++){
        for(i = 0; i < 16; i++) {
            input[player][i] = state[player][i>>2][i&0x3] & 0xFF;
        }
    }
}


void mpc_skinny_128_384_plus_enc_verify (uint8_t input[2][16], const uint8_t tweakeys[2][48], uint8_t output[2][16], randomTape_t* rand, view_t views[2]) {
  mpc_enc_verify(input, tweakeys, output, rand, views); 
}
...
// Resets the value of the counter.
void mpc_reset_lfsr_gf56_verify (unsigned char CNT[2][7]) {
    for(int player = 0;player < 2; player++){
        CNT[player][0] = 0x01;
        CNT[player][1] = 0x00;
        CNT[player][2] = 0x00;
...
// Applies CNT'=2 * CNT (mod GF(2^56)), where GF(2^56) is defined using the irreducible polynomial
// x^56 + x^7 + x^4 + x^2 + 1
void mpc_lfsr_gf56_verify (unsigned char CNT[2][7]) {
  unsigned char fb0;
    ...
    for(int player = 0;player<2;player++){
        ...
        CNT[player][6] = (CNT[player][6] << 1) | (CNT[player][5] >> 7);
        CNT[player][5] = (CNT[player][5] << 1) | (CNT[player][4] >> 7);
        CNT[player][4] = (CNT[player][4] << 1) | (CNT[player][3] >> 7);
        CNT[player][3] = (CNT[player][3] << 1) | (CNT[player][2] >> 7);
        CNT[player][2] = (CNT[player][2] << 1) | (CNT[player][1] >> 7);
        CNT[player][1] = (CNT[player][1] << 1) | (CNT[player][0] >> 7);
        if (fb0 == 1) {
            CNT[player][0] = (CNT[player][0] << 1) ^ 0x95;
        }
        else {
            CNT[player][0] = (CNT[player][0] << 1);
        }
    }
}

// Combines the secret key, counter and domain bits to form the full 384-bit tweakey
void mpc_compose_tweakey_verify (unsigned char KT[2][48],
                  const unsigned char K[2][16],
                  unsigned char T[2][16],
                  unsigned char CNT[2][7],
                  unsigned char D) 
{
  ...
  int i;
    ...
    for(int player = 0;player<2;player++){
        ...
        for (i = 0; i < 16; i++) {
            KT[player][i+16] = T[player][i];
        }
        for (i = 0; i < 16; i++) {
            KT[player][i+32] = K[player][i];
        }
    }
}

// An interface between Romulus and the underlying TBC
void mpc_block_cipher_verify(uint8_t s[2][16],
                        const unsigned char K[2][16],
                        unsigned char T[2][16],
                        unsigned char CNT[2][7], 
                        unsigned char D,
                        ...
                        view_t views[2]) 
{
  unsigned char KT[2][48];
  
  mpc_compose_tweakey_verify(KT,K,T,CNT,D);
  mpc_skinny_128_384_plus_enc_verify(s, KT, output, rand, views);
}

// Initialization function: KDF
void mpc_kdf_verify (const unsigned char K[2][16], unsigned char Z[2][16], const unsigned char N[2][16], unsigned char CNT[2][7],
                uint8_t output[2][16], randomTape_t* rand, view_t views[2]) 
{  

    int i;
    unsigned char T[2][16]={{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    ...
    mpc_block_cipher_verify(Z,K,T,CNT,66,output,rand,views); 
}


// Encrypts the message blocks.
void mpc_msg_encryption_verify (const unsigned char M[2][16], unsigned char C[2][16],
                   const unsigned char N[2][16],
                   unsigned char CNT[2][7],
                   unsigned char Z[2][16],
                    randomTape_t* rand,
                    view_t views[2],
                    paramset_t* params) 
{
    ...
    len8 = params->stateSizeBytes;

    for(player=0;player<2;player++){
        for (i = 0; i < 16; i++) {
            S[player][i] = N[player][i];
        }
    }

    mpc_block_cipher_verify(S,Z,T,CNT,64,C,rand,views);

    for(player=0;player<2;player++){
        for (i = 0; i < len8; i++) {
            C[player][i] = M[player][i] ^ S[player][i];
        }
    }

    mpc_lfsr_gf56_verify(CNT);
}

/*
* - PICNIC INPUT      | - Rolumus INPUT
* --------------------|---------------
* - plaintext         | - message
...
void mpc_romulus_t_verify (const uint8_t *plaintext,
                        randomTape_t* rand,
                        ...
                        paramset_t* params)
{
    ...
    uint8_t K[2][16];
    uint8_t C[2][16];
    unsigned char CNT_Z[2][7] = {{0x00,0x00,0x00,0x00,0x00,0x00,0x00},{0x00,0x00,0x00,0x00,0x00,0x00,0x00}};
    unsigned int player;
    size_t m_len;
    ...
    for(player=0;player<2;player++){
        memcpy(N[player], nonce, 16);
        memcpy(M[player], plaintext, 16);
        memcpy(K[player], (uint8_t*)views[player].inputShare,16);
    }
  
    mpc_reset_lfsr_gf56_verify(CNT);

    mpc_kdf_verify(K,Z,N,CNT_Z,C,rand,views);

    mpc_msg_encryption_verify(M,C,N,CNT,Z,rand,views,params);


    for(player=0;player < 2;player++){
        memcpy(views[player].outputShare, C[player], params->stateSizeBytes);
   }
}
                 ...
                 const uint32_t* plaintext, randomTape_t* tape, paramset_t* params)
{
    ...
    /*if(roundNumber == 0){
        ...
        for(int aux=0;aux<4;aux++){
            printf("\nview0[x] = %u ; view1[x+1] = %u ;",((uint8_t*)view1->inputShare)[aux],((uint8_t*)view2->inputShare)[aux]);
        }
    }*/

    mpc_romulus_t_verify((uint8_t*) plaintext, tape,view1,view2,nonce,params);
    
    /*if(roundNumber == 0){
        printf("\n\nOutput:\n");
        for(int aux=0;aux<4;aux++){
            printf("\nview0[x] = %u ; view1[x+1] = %u ;\n",((uint8_t*)view1->outputShare)[aux],((uint8_t*)view2->outputShare)[aux]);
        }
    }*/
...
static void mpc_SKINNY_AND(uint8_t in1[3], uint8_t in2[3], uint8_t out[3], randomTape_t* rand,
             view_t views[3])
{
    uint8_t r[3] = { getBit(rand->tape[0], rand->pos), getBit(rand->tape[1], rand->pos), getBit(rand->tape[2], rand->pos) };
...
static void mpc_skinny_bytes_update(uint8_t x[3], int index, int players, randomTape_t* rand, view_t views[3]){
    ...
    mpc_SKINNY_AND(a,b,ab,rand,views);
...
// Extract and apply the subtweakey to the internal state (must be the two top rows XORed together), then update the tweakey state
void mpc_AddKey(unsigned char state[3][4][4], unsigned char keyCells[3][3][4][4])
{
    int i, j, k, player;
    unsigned char pos;
    unsigned char keyCells_tmp[3][3][4][4];
...
// Apply the constants: using a LFSR counter on 6 bits, we XOR the 6 bits to the first 6 bits of the internal state
void mpc_AddConstants(unsigned char state[3][4][4], int r)
{
...
// apply the 8-bit Sbox
void mpc_SubCell8(uint8_t state[3][4][4], int index, randomTape_t* rand, view_t views[3])
{
    int i,j;
    uint8_t aux[3];
    for(i = 0; i < 4; i++){
        for(j = 0; j <  4; j++){
            ...
            mpc_skinny_bytes_update(aux,index,3,rand,views);
            /*if(debug == 2 && index == 0){
                printf("aux[0] = %u ; aux[1] = %u\n",aux[0],aux[1]);
            }*/
            state[0][i][j] = aux[0];
            state[1][i][j] = aux[1];
            state[2][i][j] = aux[2];
...
// Apply the ShiftRows function
void mpc_ShiftRows(unsigned char state[3][4][4])
{
    int i, j, pos, player;

    unsigned char state_tmp[3][4][4];
    ...
    for(player=0; player<3; player++){
        ...
        for(i = 0; i < 4; i++)
        {
            for(j = 0; j < 4; j++)
            {
                //application of the ShiftRows permutation
                pos=P[j+4*i];
                state_tmp[player][i][j]=state[player][pos>>2][pos&0x3];
            }
        }
        ...
        for(i = 0; i < 4; i++)
        {
            for(j = 0; j < 4; j++)
            {
                state[player][i][j]=state_tmp[player][i][j];
            }
        }
    }
}
...
//0 1 1 0
//1 0 1 0
void mpc_MixColumn(unsigned char state[3][4][4])
{
    int j, player;
    unsigned char temp;
    for(player=0; player<3; player++){
...
// encryption function of Skinny
void mpc_enc(uint8_t input[3][16], const uint8_t userkey[3][48], uint8_t output[3][16], randomTape_t* rand, view_t views[3])
{
    unsigned char state[3][4][4];
    unsigned char keyCells[3][3][4][4];
    int i, player;
    (void) output;
    ...
    for(player= 0; player<3; player++){ 
        ...
        for(i = 0; i < 16; i++) {
            ...
            keyCells[player][0][i>>2][i&0x3] = userkey[player][i]&0xFF;
            keyCells[player][1][i>>2][i&0x3] = userkey[player][i+16]&0xFF;
            keyCells[player][2][i>>2][i&0x3] = userkey[player][i+32]&0xFF;
...
void mpc_skinny_128_384_plus_enc (uint8_t input[3][16], const uint8_t tweakeys[3][48], uint8_t output[3][16], randomTape_t* rand, view_t views[3]) {
  mpc_enc(input, tweakeys, output, rand, views); 
}
...
// Resets the value of the counter.
void mpc_reset_lfsr_gf56 (unsigned char CNT[3][7]) {
    for(int player = 0;player < 3; player++){
        CNT[player][0] = 0x01;
        CNT[player][1] = 0x00;
        CNT[player][2] = 0x00;
...
// Applies CNT'=2 * CNT (mod GF(2^56)), where GF(2^56) is defined using the irreducible polynomial
// x^56 + x^7 + x^4 + x^2 + 1
void mpc_lfsr_gf56 (unsigned char CNT[3][7]) {
  unsigned char fb0;
...
// Combines the secret key, counter and domain bits to form the full 384-bit tweakey
void mpc_compose_tweakey (unsigned char KT[3][48],
                  const unsigned char K[2][16],
                  unsigned char T[2][16],
                  unsigned char CNT[3][7],
                  unsigned char D) 
{
...
// An interface between Romulus and the underlying TBC
void mpc_block_cipher(unsigned char s[3][16],
            const unsigned char K[2][16],
            unsigned char T[2][16],
            unsigned char CNT[3][7], 
            unsigned char D,
            ...
            view_t views[3]) 
{
  unsigned char KT[3][48];

  mpc_compose_tweakey(KT,K,T,CNT,D);
  mpc_skinny_128_384_plus_enc(s,KT,output,rand,views);
}

// Initialization function: KDF
void mpc_kdf (const unsigned char K[3][16], unsigned char Z[3][16], const unsigned char N[3][16], unsigned char CNT[3][7],
            uint8_t C[3][16], randomTape_t* rand, view_t views[3]) 
{  

    int i;
    unsigned char T[3][16]={{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
                            {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
                            {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}};
    ...
    mpc_block_cipher(Z,K,T,CNT,66,C,rand,views); 
}


// Encrypts the message blocks.
void mpc_msg_encryption (const uint8_t M[3][16], uint8_t C[3][16],
                    const unsigned char N[3][16],
                    unsigned char CNT[3][7],
                    unsigned char Z[3][16],
                    randomTape_t* rand,
                    view_t views[3],                   
                    paramset_t* params) 
{
    ...
    mpc_block_cipher(S,Z,T,CNT,64,C,rand,views);
...
void mpc_romulus_t (const uint8_t *plaintext,
                        randomTape_t* rand,
                        view_t views[3],
                        const uint8_t *nonce,
                        paramset_t* params)
{
    unsigned char Z[3][16];
    unsigned char CNT[3][7];
    unsigned char CNT_Z[3][7] = {{0x00,0x00,0x00,0x00,0x00,0x00,0x00},
                                 {0x00,0x00,0x00,0x00,0x00,0x00,0x00},
                                 {0x00,0x00,0x00,0x00,0x00,0x00,0x00}};
    uint8_t M[3][16];
    uint8_t C[3][16];
    unsigned char N[3][16];
    size_t m_len;
    ...
    mpc_reset_lfsr_gf56(CNT);

    // Prepare each player's K
    mpc_kdf(K,Z,N,CNT_Z,C,rand,views);
    
    mpc_msg_encryption(M,C,N,CNT,Z,rand,views,params);

    for(int player=0;player < 3;player++){
        uint8_t* view_aux = (uint8_t*) views[player].outputShare;
        memcpy(view_aux, C[player], params->stateSizeBytes);
    }
}
                 ...
                 size_t messageByteLength, signature_t* sig, paramset_t* params)
{
    ...
    for (uint32_t k = 0; k < params->numMPCRounds; k++) {
        ...
        uint8_t nonce[] = "000102030405060708090A0B";
        mpc_romulus_t((uint8_t*)plaintext,&tape,views[k],nonce,params);
...
int serializeSignature(const signature_t* sig, uint8_t* sigBytes, size_t sigBytesLen, paramset_t* params)
{
    ...
    printf("Salt size bytes   => %u\n", params->saltSizeBytes);
    printf("Seed size bytes   => %u\n", params->numMPCRounds * 2 * params->seedSizeBytes);
    printf("View Input bytes  => %u\n",params->numMPCRounds * params->stateSizeBytes);
    printf("Single and size bytes => %u\nAND size bytes    => %u\n",params->andSizeBytes,params->numMPCRounds * params->andSizeBytes);
    printf("View Output bytes => %u\n",params->numMPCRounds * params->digestSizeBytes);
